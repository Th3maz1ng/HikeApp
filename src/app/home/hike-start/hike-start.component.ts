      ...
      Object.keys(e.target._layers).forEach((key) => {
          Object.keys(e.target._layers[key]._layers).forEach((key2) => {
            if(e.target._layers[key]._layers[key2]['_popup']) {
              ...
              const lat = e.target._layers[key]._layers[key2]['_latlng']['lat'];
              const lng = e.target._layers[key]._layers[key2]['_latlng']['lng'];
              const test = new L.LatLng(lat,lng);
              const cercle = new L.Circle(test,{
                radius: 100,
                color: 'red',
                opacity: 0,
